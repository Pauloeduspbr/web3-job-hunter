...
**Senior Data Engineer | Data Platform Engineering | Real-Time Streaming, Lakehouse & AWS | FinTech & Digital Assets**

*ATS Match: 90.0% — Data Platform Engineering Manager @ kraken*
...
Senior Data Engineer with 9 years building and operating large-scale **data platforms** for banks, payments, telecom, healthcare, and e-commerce organizations that process millions of daily transactions. Specialized in **AWS** data-lakes and lakehouse architecture (**Spark**, **EMR**/EMR Serverless, **Athena**, **Iceberg**, **Parquet**, **S3**, **Glue**, **Trino**), **dimensional modeling**, **data quality** best practices, and real-time / **streaming** data pipelines (**Kafka**, Kinesis, **CDC**, event-driven).

Hands-on in **Python**, **SQL**, **PySpark/Spark**, and **Scala**, with technical leadership and mentoring of engineering teams across multi-stakeholder delivery (product, finance, compliance, analytics). Proven results: **99.9% uptime SLA**, **60% storage reduction**, **50% faster processing**, **70% less manual operation**, and **40% maintenance reduction**. Builds platform capabilities that serve downstream analytics and **machine learning** consumers, and maintains public open-source **blockchain data** projects (Ethereum, Solana, DeFi) bridging enterprise data engineering with Web3/crypto and high-throughput financial data.

---

## TECHNICAL SKILLS

**Languages:** Python (advanced), SQL (advanced), PySpark, Scala, Shell Script

**AWS (cloud-native data platform):** S3, EMR / EMR Serverless, Glue, Athena, Trino/Presto, Redshift, Lambda, Kinesis, Lake Formation, DMS, Step Functions, KMS, CloudWatch

**Streaming & Real-Time:** Apache Kafka, Kinesis, CDC (Change Data Capture), Debezium-style ingestion, Spark Streaming, event-driven architecture, Apache NiFi

**Lakehouse & Formats:** Apache Iceberg, Delta Lake, Apache Hudi, Parquet

**Processing & Orchestration:** Apache Spark, PySpark, SparkSQL, Apache Airflow, dbt, Talend, DataStage, SSIS, Informatica PowerCenter/BDM, Sqoop, Flume

**Architecture & Modeling:** Data Lake & Lakehouse (Medallion Bronze/Silver/Gold), Data Warehouse (Star/Snowflake schema), Dimensional Modeling, CDC, Event-Driven, Cloud Migration (on-prem → cloud), Big Data (Hadoop/Spark)

**Governance & Data Quality:** Data Quality frameworks, Data Governance, Data Catalog (AWS Glue, Unity Catalog), self-service tooling, PII/LGPD compliance, HashiCorp Vault, AWS KMS, Fernet encryption

**Other Cloud:** Azure (Data Lake, Databricks, Data Factory, Synapse, Unity Catalog, Fabric) · GCP (BigQuery, Dataproc)
...
**Adjacent (working knowledge):** Machine Learning fundamentals, AI/automation in data workflows (AI-assisted pipeline tooling)

**Methodologies & Leadership:** Agile for Analytics (Scrum, SAFe), technical leadership and mentoring of engineers, requirements gathering for analytics projects
...
- Built a healthcare Data Lake with Medallion architecture (Bronze/Silver/Gold) on **Apache Iceberg** over **S3**, serving analytical and downstream consumers via **Trino**, **Athena**, and Redshift, supporting electronic health records, scheduling, vaccines, medication, prescriptions, and tele-consultation use cases (Amplimed, HAOC, Vitat).
- Developed **PySpark/SparkSQL** pipelines for large-scale ingestion, transformation, and consolidation of clinical data, with support for **Apache Iceberg** and Apache Hudi over **Parquet**.
- Implemented an in-house **Data Quality** framework (cross-origin validations, light checks, DLS validations) and a YAML-versioned, self-service data catalog.
- Modeled the Customer 360 analytical table (dimensional modeling) integrating multiple sources (CRM, scheduling, events, interests), served to analytics and machine learning consumers.
...
- Integrated API + streaming-style ingestion (Amplimed, HAOC, Precifica) with async S3 clients and evolving schema handling (JSON/Parquet/Iceberg).
...
- Implemented a high-availability NiFi cluster integrated with Active Directory; configured an Application Load Balancer and built NiFi flows for SAP ingestion and S3↔Redshift movement (event-driven, streaming-style data movement).
...
- **crypto-data-pipeline** — production-style pipeline ingesting market data for 100 crypto assets (CoinGecko API) with Airflow, PostgreSQL, and Docker; 1.74s execution; 7/7 automated data quality checks passing.
